...
pub struct InputFieldsDefinition<'a, S: SchemaDefinition, W: Warden<SchemaDefinition = S>> {
    inner: &'a S::InputFieldsDefinition,
    cache: &'a Cache<'a, S, W>,
    input_fields_definition: OnceCell<Vec<InputValueDefinition<'a, S, W>>>,
}

impl<'a, S: SchemaDefinition, W: Warden<SchemaDefinition = S>> InputFieldsDefinition<'a, S, W> {
    pub(crate) fn new(inner: &'a S::InputFieldsDefinition, cache: &'a Cache<'a, S, W>) -> Self {
        Self {
            inner,
            cache,
...
impl<'a, S: SchemaDefinition + 'a, W: Warden<SchemaDefinition = S>> AsIter
    for InputFieldsDefinition<'a, S, W>
{
