...
use crate::validation::executable::{Error, Rules, Rule, Visitor};
...
impl<'a, E: ExecutableDocument<'a>, S: SchemaDefinition<'a>> Validator<'a, E, S> {
    ...
    fn visit(&mut self) {
        self.executable_document.operation_definitions().iter().for_each(|operation_definition| {
            self.visit_operation_definition(operation_definition.as_ref());
        });
        self.executable_document.fragment_definitions().iter().for_each(|fragment_definition| {
            self.visit_fragment_definition(fragment_definition);
        });
    }

    fn visit_operation_definition(&mut self, operation_definition: &'a OperationDefinitionFromExecutableDocument<'a, E>) {
        ...
        self.visit_selection_set(
            ...
            self.schema_definition.get_type(self.schema_definition.query().name()).expect("Schema definition's `get_type` method returned `None` for query root"),
        )
    }

    fn visit_fragment_definition(&mut self, fragment_definition: &'a E::FragmentDefinition) {
        let type_condition = self.schema_definition.get_type(fragment_definition.type_condition());
        if let Some(type_condition) = type_condition {
            self.visit_selection_set(fragment_definition.selection_set(), type_condition);
        }
    }

    fn visit_selection_set(&mut self, selection_set: &'a E::SelectionSet, scoped_type: &'a TypeDefinitionReferenceFromAbstract<S::TypeDefinitionReference>) {
    ...
    pub fn validate(executable_document: &'a E, schema_definition: &'a S) -> <Self as IntoIterator>::IntoIter {
