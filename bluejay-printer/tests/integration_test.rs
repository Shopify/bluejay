...
fn test_definition_printer() {
    ...
    let original_document: DefinitionDocument =
        DefinitionDocument::parse(s.as_str()).unwrap().into_parsed();
    ...
    let printed_document: DefinitionDocument = DefinitionDocument::parse(printed.as_str())
        .unwrap()
        .into_parsed();
