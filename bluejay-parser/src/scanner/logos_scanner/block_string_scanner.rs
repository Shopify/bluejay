...
impl<'a> Token<'a> {
    pub(super) fn parse(s: &'a <Self as Logos<'a>>::Source) -> Result<(String, usize), ()> {
        ...
        while let Some(token) = lexer.next() {
            match token {
                ...
                Self::BlockStringCharacters(_) | Self::EscapedBlockQuote | Self::Whitespace(_) => lines.last_mut().unwrap().push(token),
    ...
    fn block_string_value(lines: Vec<Vec<Token<'a>>>) -> String {
        ...
        for line in &lines[1..] {
            let leading_whitespace = line.iter().position(|token| !matches!(token, Self::Whitespace(_)));
        ...
        let front_offset = lines.iter().enumerate().position(|(idx, line)| {
            ...
            line[min(line.len(), indent)..].iter().any(|token| !matches!(token, Token::Whitespace(_)))
        });

        let end_offset = lines.iter().rev().position(|line| {
            line[min(line.len(), common_indent)..].iter().any(|token| !matches!(token, Token::Whitespace(_)))
        });
        ...
        if let Some(front_offset) = front_offset {
            if let Some(end_offset) = end_offset {
                ...
                formatted.extend(
                    lines[start..end].iter().enumerate().flat_map(|(offset_idx, line)| {
                        ...
                        std::iter::once(prepend).chain(
                            line[min(line.len(), indent)..].iter().filter_map(|token| match token {
